...
//ver que no supere tam archmax
 ...
 byterx=recv(socket_desc, convertir.buffer , sizeof(struct archivo) , 0);

strcat(convertir.dato.nombre,"M");

 fdw = open(convertir.dato.nombre, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH );
    ...
    write(fdw, (convertir.buffer+256+sizeof(unsigned long int)) ,convertir.dato.tam );
